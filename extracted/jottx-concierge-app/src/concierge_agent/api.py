...
async def chat(req: ChatRequest):
    ...
    if llm_client.LLM_ENABLED:
        ...
        if 'final' in result:
            raw_reply = result['final']
        else:
            raw_reply = str(result)

        # Try to extract nested Ollama response JSON if present
        def extract_llama_response(s: str) -> str:
            try:
                j = json.loads(s)
                # Ollama may return {'response': '...'} or embed as string
                if isinstance(j, dict) and 'response' in j:
                    try:
                        # response may itself be JSON string
                        inner = json.loads(j['response'])
                        # prefer human-friendly string if present
                        if isinstance(inner, dict):
                            # try common fields
                            if 'deals' in inner:
                                return 'Found ' + str(len(inner['deals'])) + ' bundles. Ask to see details.'
        ...
        reply = extract_llama_response(raw_reply)
    else:
        # fallback rule-based
