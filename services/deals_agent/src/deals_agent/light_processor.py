...
OUT_TOPIC = os.environ.get('PRODUCE_TO_TOPIC')
...
async def normalize_record(raw: dict) -> dict:
    ...
    # If LLM is enabled, ask it to suggest additional tags or refine tags
    if llm_client.LLM_ENABLED:
        try:
            prompt = (
                f"Given this listing raw data: {raw}.\n\nReturn a JSON array of short tag strings (lowercase), e.g. [\"wifi\", \"breakfast\"]."
            )
            llm_out = await llm_client.generate(prompt, max_tokens=64, temperature=0.0)
            # try to parse JSON array from LLM output
            try:
                parsed = json.loads(llm_out)
                if isinstance(parsed, list):
                    # merge and dedupe
                    tags = list(dict.fromkeys([t.lower() for t in tags + parsed]))
            except Exception:
                # fallback: split on commas and extract words
                try:
                    extra = [t.strip().lower() for t in llm_out.replace('[','').replace(']','').split(',') if t.strip()]
                    tags = list(dict.fromkeys([t.lower() for t in tags + extra]))
                except Exception:
                    pass
        except Exception as e:
            print('LLM tag generation failed', e)
...
async def process_normalized(normalized: dict):
    ...
    # produce to Kafka if configured (best-effort, transient producer)
    if KAFKA_BOOTSTRAP and OUT_TOPIC:
        try:
            ...
            print('kafka produce failed (light_processor):', e)
