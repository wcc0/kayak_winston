    ...
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        <Box>
          ...
          <Typography variant="subtitle1" color="textSecondary">Overview of users, revenue and bookings</Typography>
        </Box>
        <Box>
          <Button variant="contained" color="primary" sx={{ mr: 1 }}>Create Report</Button>
        ...
        <Grid item xs={12}>
          <Paper sx={{ p: 3 }}>
            ...
            <Box sx={{ mt: 2, display: 'flex', gap: 1 }}>
              <Button variant="contained" color="primary" onClick={() => window.location.href = '/user/signup'}>Create User</Button>
              <Button variant="outlined" onClick={() => window.location.href = '/user/login'}>User Login</Button>
            </Box>
          </Paper>
        </Grid>
      </Grid>
      {/* Agent chat button (user-facing) has been moved to the User Dashboard */}
    </Container>
  );
};
